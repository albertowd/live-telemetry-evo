...
[*]Engine power (HP) and torque, live when the game publishes it, or interpolated from the car's power curve.
[*]Boost pressure (bar), hidden on naturally-aspirated cars.
[*]Gear and speed (km/h).
[*]Driver-aid status chips: pit limiter, TC, ABS, ESC, launch control, DRS, ERS, wrong way, invalid lap, last lap.
[*]Analog readouts: water temp, oil temp, oil pressure, fuel pressure, exhaust temp, battery voltage, fuel, brake bias.
...
[*]Suspension travel and per-axle ride height (mm).
...
[*][B]AC1[/B] — live data plus a synthesised power curve for HP, slip-threshold heuristics for lock / ABS blink, curve-interpolated normalised tire temps, live tire wear bar.
...
The HP label shows the current live horsepower when the game publishes it, otherwise the value is interpolated from the bundled power curve and scaled by the current boost ([COLOR=rgb(44, 130, 201)]hp = power * ( 1 + boost )[/COLOR]). Gear, speed (km/h) and RPM sit on the same row.

The boost bar above the RPM bar:

[LIST]
[*]white: boost below 90 % of the car's maximum.
[*][COLOR=rgb(97, 189, 109)]green[/COLOR]: boost above 90 %.
[/LIST]

A strip of driver-aid chips appears only while each condition is true so the bar stays compact: pit limiter, TC (bright when cutting, dim when armed), ABS (same scheme), ESC, launch control, DRS (bright when deployed, dim when only available), ERS charging, wrong-way warning, invalid-lap and last-lap markers.

The bottom row shows water / oil temperature, oil and fuel pressure, exhaust temperature, battery voltage, fuel litres, and brake bias percentage. Cells that the running game does not publish are hidden automatically.
...
[/LIST]
[*][B]Tire wear[/B] (%) — AC1 only. Vertical bar fills with remaining grip.
[LIST]
[*][COLOR=rgb(97, 189, 109)]green[/COLOR]: above 50 %.
[*][COLOR=rgb(247, 218, 100)]yellow[/COLOR]: 20 % – 50 %.
...
[*][B]Suspension bar[/B] — tinted graphic on the outer side of the wheel. The bar fills at full extension and shrinks as the suspension compresses (kept from the original LiveTelemetry plugin for parity). When the game does not publish a per-car maximum travel, the bar self-calibrates against the maximum observed in the session.
[LIST]
[*][COLOR=rgb(226, 226, 226)]white[/COLOR]: mid-travel.
...
[/LIST]
[*][B]Brake disc temperature[/B] — top-inner icon tinted by disc temperature (peak grip around 400 ºC).
[LIST]
...
[*][B]Wheel lock / ABS[/B] — the brake icon blinks to flag pedal events on this corner.
[LIST]
[*][COLOR=rgb(44, 130, 201)]blue[/COLOR]: ABS modulating on this wheel.
[*][COLOR=rgb(247, 218, 100)]yellow[/COLOR]: wheel locked up (mostly cars with no ABS).
[*][COLOR=rgb(226, 226, 226)]white[/COLOR]: regular braking, no event.
[/LIST]
...
A system-tray icon mirrors the same options (reset positions, click-through toggle, size submenu, quit), with the hotkey shown next to each label.
...
All overlay state lives in a single JSON file at [COLOR=rgb(44, 130, 201)]%APPDATA%\LiveTelemetryEvo\Overlay\positions.json[/COLOR]. It stores per-widget positions and visibility plus the chosen size index. A saved position is honoured on next launch only if the widget would land fully on screen at the current resolution; otherwise it falls back to the layout default.
...
[*]Double-click the executable. A [I]Detecting AC Environment…[/I] screen stays up until the overlay finds the game; once it attaches, a 5-second countdown plays and the widgets reveal.
[*]Use [B]Ctrl+Alt+L[/B] to unlock for repositioning, then drag widgets where you want them.
[/LIST]

To uninstall, just delete the [COLOR=rgb(44, 130, 201)].exe[/COLOR] and (optionally) the [COLOR=rgb(44, 130, 201)]%APPDATA%\LiveTelemetryEvo\[/COLOR] folder.
