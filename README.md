...
[![Overlay running on top of AC Evo](https://raw.githubusercontent.com/albertowd/live-telemetry-evo/main/resources/previews/ac-evo.webp)](https://youtu.be/N8AROyFXrBs)
...
| Suspension travel bar | live (session max) | live (car limit, else session max) | live (session max) | live (car limit, else session max) |
...
| Readouts    | Bottom row: `WAT` water temp °C, `OIL` oil temp °C, `OPR` oil pressure bar, `FPR` fuel pressure bar, `EXH` exhaust temp °C, `BAT` battery V, `BATT` battery temp °C (AC EVO), `FUEL` litres, `BIAS` brake bias (%F front — on ACC the per-car dash offset from the official documentation is applied, so the value matches the in-car display). Cells whose source publishes nothing for the current car are hidden so the strip shows only live values. |
...
| Suspension bar        | Tinted suspension graphic on the **outer** side of the wheel, showing live compression against a reference travel. The inner fill is the original AC plugin convention — the bar **fills at full extension (droop) and shrinks as the suspension compresses** (height ∝ `1 − travel_ratio`). Counter-intuitive vs a typical "load grows" gauge but kept for parity.<br><br>**What the reference travel is.** Two modes, and the mid-band colour tells you which: **white** = the car data's total mechanical travel (`static.suspensionMaxTravel`), fixed per car; **blue** = self-calibrating, the deepest compression seen so far for this car. **Yellow** outside ±10 % of that reference, **red** outside ±5 %. AC EVO dropped the field from its static block and ACC's documentation marks it *"Not shown in ACC"*, so both are effectively always self-calibrating; AC1 and AC Rally supply it for cars whose data carries it (Kunos cars usually do, mods often don't). The reference resets when you change car.<br><br>**It is not your setup.** No AC title publishes bump-stop or packer position — the only suspension telemetry is current compression in metres (plus damage). Reducing the travel/packer range in the setup screen therefore can't shrink the bar's scale; it changes how much of the bar you use. Likewise, `red` means "near the reference", which only equals "on the bump-stops" when the car supplied a real mechanical limit.<br><br>**Caveat in self-calibrating mode.** The reference is a one-way high-water mark: a single kerb strike, landing or off-track drop can set it for the rest of the session, after which normal driving never approaches it and the yellow/red bands stop appearing. Measured on a real AC1 stint, one off-track kerb hit raised a corner's reference by 21 % and left only 2 frames in 12 458 inside the red band. |
...
| red    | past peak (or shift-up hint) | above ideal (hot / overpressure / hot brake) | bad — locked, fully worn, or suspension within ±5 % of its travel reference |
